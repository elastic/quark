...
event_storage_clear(struct quark_queue *qq)
{
	...
	qq->event_storage.pod = NULL;
	qq->event_storage.container = NULL;
}
...
quark_event_dump(const struct quark_event *qev, FILE *f)
{
	...
	if (container != NULL) {
		flagname = "CONT";
		P("  %.4s\tname=%s image=%s\n",
		    flagname, container->name, container->image);
		P("  %.4s\tcontainer_id=%s\n", flagname,
...
quark_queue_default_attr(struct quark_queue_attr *qa)
{
	...
	qa->kubefd = -1;		/* disabled by default */
}
...
#define GET cJSON_GetObjectItemCaseSensitive
	...
	/*
	 * When we're waiting there's no containerID yet, so supress it
	 */
...
#define GET cJSON_GetObjectItemCaseSensitive
	...
	cJSON			*deletionTimestamp, *containerStatuses;
...
//	cJSON			*containerID;
...
#if 0
	if (!strcmp(phase, "Pending")) {
		printf("discarding pending...\n");
		return (0);
	}
#endif
	...
	if (pod == NULL) {
		new_pod = 1;
		printf("pod %s NOT found, adding it\n", uid->valuestring);
		if (0)
			debug_json(json);
		...
		if (pod == NULL)
			return (-1);
		/*
		 * Only fill in uid which is the key, rest is filled and/or
	...
	/* Mutable data */
	/* XXX we might be being too careful */
...
parse_kube_events(struct quark_queue *qq)
{
	...
	if (ev_len > (qkube->buf_len - sizeof(ev_len))) /* TODO MAXMSG = 1 << 17 - 4 */
		errx(1, "XXX BUG3");
	/* Partial event */
	...
	if ((json = cJSON_ParseWithLength(ev, ev_len)) == NULL) {
		qwarnx("can't create json of event (len=%d)\n", ev_len);
		return (1);
	}
...
/* XXX TODO add this to scrape/iterators */
static void
add_kube_data(struct quark_queue *qq, struct quark_event *qev)
{
//	struct quark_kube		*qkube = qq->qkube;
	const struct quark_process	*qp;
//	struct quark_pod		*pod;
	...
	if (!(qp->flags & QUARK_F_CGROUP))
		return;
	if ((name = basename(qp->cgroup)) == NULL)
		return;
	/* docker-f6aa2e3fa923d32f4d7905727cf1011148e4da0fd101492e98a27e8c55c5c829.scope */
        /* "containerID": "docker://4448260ce79343883fe542aad8bc54d114b739c35efdc20da823e180b6158a82", */
	...
	if (dot == NULL)
		return;
	*dot = 0;
	/* strlcpy(container_id, "docker://", sizeof(containerID)); */
	/* strlcat(container_id, qp->cgroup, sizeof(container_id)) */
	printf("cgroup is %s\n", qp->cgroup);
	printf("looking up for %s\n", container_id);
	container = container_lookup(qq, container_id);
	if (container == NULL)
		printf("NOT found container %s\n", container_id);
	else
		printf("FOUND! container %s pod=%s\n",
		    container->name, container->pod->name);

	if (container != NULL) {
		qev->container = container;
		qev->pod = container->pod;
	}
}
...
quark_queue_get_event(struct quark_queue *qq)
{
	...
	if (qev != NULL && qq->qkube != NULL)
		add_kube_data(qq, (struct quark_event *)qev);
