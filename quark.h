 ...
 * it attaches. QUARK_TAMPER_ATTACH_TYPE is a legacy cgroup detach that names
 * no object of ours, id is then the attach type. name is owned by the queue
 * and valid until quark_queue_close(3).
 */
#define QUARK_TAMPER_MAP		1
#define QUARK_TAMPER_PROG		2
#define QUARK_TAMPER_LINK		3
#define QUARK_TAMPER_ATTACH_TYPE	4
#define QUARK_TAMPER_F_KEY		(1 << 0)	/* key is valid */
struct quark_tamper {
	u32		 kind;		/* QUARK_TAMPER_{MAP,PROG,LINK,ATTACH_TYPE} */
	u32		 id;		/* kernel id, or attach type */
