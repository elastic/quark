...
static struct file *file_from_fd(u32 fd)
{
    const struct task_struct *task = (struct task_struct *)bpf_get_current_task();
...
static bool file_dname_is(const struct file *file, const char *want)
{
    const unsigned char *name = BPF_CORE_READ(file, f_path.dentry, d_name.name);
    char buf[BPF_DNAME_LEN];
    int i;

    if (!name || bpf_probe_read_kernel_str(buf, sizeof(buf), name) < 0)
        return false;
#pragma unroll
