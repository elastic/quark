 ...
 * type, sizes, entries and name (bpf_map_info.name is truncated to 15
 * characters, so every elastic_ebpf_events_* map shares one). A zero
 * value_size or max_entries matches any. Returns an fd and fills info.
 */
static int
    ...
    u32 max_entries, const char *name)
{
	...
	while (bpf_map_get_next_id(id, &id) == 0) {
		...
		if (info->type == type &&
		    ...
		    (max_entries == 0 || info->max_entries == max_entries) &&
...
find_trusted_map(void)
{
	...
	fd = find_map(&info, BPF_MAP_TYPE_HASH, sizeof(u32), sizeof(u32), 512,
...
t_trusted_map_rdonly(const struct test *t, struct quark_queue_attr *qa)
{
	...
	fd = find_trusted_map();
	/* Kernels since 6.18 read hash and hash_size back, garbage is EINVAL */
	bzero(&info, sizeof(info));
	if (bpf_map_get_info_by_fd(fd, &info, &len) != 0)
		err(1, "bpf_map_get_info_by_fd");
...
t_map_freeze(const struct test *t, struct quark_queue_attr *qa)
{
	...
	fd = find_map(&info, BPF_MAP_TYPE_HASH, sizeof(void *), sizeof(u32), 0,
	    "sk_to_tgid");
	key64 = 0xdeadbeef;
	v = 1;
	...
	fd = find_map(&info, BPF_MAP_TYPE_PERCPU_ARRAY, sizeof(u32), 0, 1,
...
t_tamper(const struct test *t, struct quark_queue_attr *qa)
{
	...
	if ((child = fork()) == -1)
		...
		/*
		 * Locate sk_to_tgid before touching the trusted map, so no
		 * reconnaissance of the trusted map happens after the delete.
		 */
		sfd = find_map(&info, BPF_MAP_TYPE_HASH, sizeof(void *),
		    sizeof(u32), 0, "sk_to_tgid");
		...
		(void)bpf_map_update_elem(sfd, &key64, &v, BPF_ANY);

		/*
		 * Finally poke every BTF object on the host, ours included. A
		 * btf fd is a bpf object too, but not one of ours, and used to be
		 * mistaken for a map when its header aliased a map id. Nothing
		 * below may be reported.
		 */
		{
			struct bpf_btf_info	binfo;
			u32			bid, blen;
			int			bfd;

			bid = 0;
			while (bpf_btf_get_next_id(bid, &bid) == 0) {
				if ((bfd = bpf_btf_get_fd_by_id(bid)) < 0)
					continue;
				bzero(&binfo, sizeof(binfo));
				blen = sizeof(binfo);
				(void)bpf_btf_get_info_by_fd(bfd, &binfo, &blen);
				close(bfd);
			}
		}
		_exit(0);
	...
	/*
	 * The walks over map ids touch every one of our maps, so there is a
	 * GET_FD_BY_ID and an OBJ_GET_INFO_BY_FD per map per walk; every
	 * reported map must be one we can name and those two are the only
	 * commands allowed on maps other than the two the child writes. On the
	 * trusted map we expect reconnaissance, possibly twice since both walks
	 * may pass it, then the insert and the delete, then the refused write on
	 * sk_to_tgid. drain_for_pid() times out if any is missing.
	 */
	seen = 0;
	while (seen != 0x1f) {
		qev = drain_for_pid(&qq, child);
	...
	/* The BTF walk came last and must have produced nothing */
	msleep(qa->hold_time * 10);
	while ((qev = quark_queue_get_event(&qq)) != NULL) {
